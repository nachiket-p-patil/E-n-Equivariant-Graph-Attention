...
class E_GCL(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, nodes_att_dim=0, act_fn=nn.ReLU(), recurrent=True, coords_weight=1.0, attention=False, clamp=False, norm_diff=False, tanh=False, nheads=4, dropout=0.1):
        ...
        self.W_query = nn.Linear(input_nf, nheads * self.d_k, bias=False)
        self.W_key = nn.Linear(input_nf, nheads * self.d_k, bias=False)
        self.W_value = nn.Linear(input_nf, nheads * self.d_k, bias=False)
        self.fc = nn.Linear(nheads * self.d_k, input_nf, bias=False)
        self.layer_norm = nn.LayerNorm(input_nf, eps=1e-6)
    ...
    def node_model(self, x, edge_index, edge_attr, node_attr, q, k, v):

        if self.attention:
            len_q, len_k, len_v = q.size(0), k.size(0), v.size(0)
            residual = q
            q = self.W_query(q).view(len_q, self.nheads, self.d_k)
            k = self.W_key(k).view(len_k, self.nheads, self.d_k)
            v = self.W_value(v).view(len_v, self.nheads, self.d_k)

            q, k, v = q.transpose(0, 1), k.transpose(0, 1), v.transpose(0, 1)

            attn = torch.matmul(q / self.d_k ** 0.5, k.transpose(1, 2))
            ...
            row, col = edge_index
            agg = unsorted_segment_sum(edge_attr, row, num_segments=q.size(0))
            if node_attr is not None:
                agg = torch.cat([q, agg, node_attr], dim=1)
            else:
