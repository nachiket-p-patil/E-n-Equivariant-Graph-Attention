from models.gcl import E_GCL, unsorted_segment_sum, unsorted_segment_mean
import torch
...
class E_GCL_mask(E_GCL):
    ...
    def forward(self, h, edge_index, coord, node_mask, edge_mask, edge_attr=None, node_attr=None, n_nodes=None):
        ...
        h, agg = self.node_model(h, edge_index, edge_feat, node_attr, h, h, h)
...
class EGNN(nn.Module):
    ...
    def forward(self, h0, x, edges, edge_attr, node_mask, edge_mask, n_nodes):
        ...
        for i in range(0, self.n_layers):
            if self.node_attr:
                h, coord, _ = self._modules["gcl_%d" % i](h, edges, coord, node_mask, edge_mask, edge_attr=edge_attr, node_attr=h0, n_nodes=n_nodes)
            else:
                h, coord, _ = self._modules["gcl_%d" % i](h, edges, coord, node_mask, edge_mask, edge_attr=edge_attr,
