...
def process_xyz_gdb9(datafile):
    ...
    for line in mol_xyz:
        atom, posx, posy, posz, charg = line.replace('*^', 'e').split()
        pos = list(map(float, [posx, posy, posz]))
        dipole_charge.append(float(charg)*np.array(pos))
        atom_charges.append(charge_dict[atom])
        atom_positions.append([float(posx), float(posy), float(posz)])

    mol_dipole = np.sum(dipole_charge, axis = 0)
    ...
    mol_props['mu'] = mol_dipole

    molecule = {'num_atoms': num_atoms, 'charges': atom_charges, 'positions': atom_positions}
    molecule.update(mol_props)
    molecule = {key: torch.tensor(val, dtype=torch.float) for key, val in molecule.items()}
